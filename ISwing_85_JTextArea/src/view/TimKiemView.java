...
public class TimKiemView extends JFrame{
	...
	private void init() {
		...
		jTextArea_vanBan = new JTextArea(100, 100); // box để nhập văn bản có số lượng dòng và số lượng cột 
		jTextArea_vanBan.setFont(font);
	
		
		
		// Footer
		TimKiemListener timKiemListener = new TimKiemListener(this); // gọi ra để nghe và thực thi hành động tương tác từ ng dùng 
		
		JLabel jLabel_tuKhoa = new JLabel("Từ khóa"); 
		jLabel_tuKhoa.setFont(font);
		jTextField_tuKoa = new JTextField(); // ko có khai báo biến là biến toàn cục, những hàm khác có thể thay đổi được giá trị	
