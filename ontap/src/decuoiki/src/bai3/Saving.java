...
public class Saving extends JPanel {
	...
	JLabel id,name,interestrate,currentbalance;
	...
	public Saving() {
		...
		butCheck.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (minimunbalanceT.getText().isBlank() || idT.getText().isBlank()|| ca.isBlank()||currentbalanceT.getText().isBlank() ) {
					JOptionPane.showMessageDialog(Saving.this, "Bạn đã nhập thiếu dữ liệu", "Lỗi nhập liệu", JOptionPane.WARNING_MESSAGE);
					return;
				}					
				try {
					...
					JOptionPane.showMessageDialog(Saving.this, "Bạn đã nhập sai kiểu dữ liệu", "Lỗi nhập liệu", JOptionPane.WARNING_MESSAGE);
					return;
				}
				area.append("\n" + ia + "\t" + na + "\t" + ma + "\t" + ca);
		...
		botPanel.setBorder(BorderFactory.createTitledBorder(border, "Saving Account Information") );
		area=new JTextArea();
		area.setText("ID"+"\t"+"Name"+"\t"+"IR[%]"+"\t"+"CB");
		botPanel.add(area);
